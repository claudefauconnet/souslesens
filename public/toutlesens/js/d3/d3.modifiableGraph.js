...
var palette0 = [ '#0056B3', '#007DFF', '#A84F02', '#A8A302', '#B354B3',
...
function drawModifiableGraph(json){
    ...
    for (var i = 0; i < resultArray.length; i++) {
        ...
        for (var j = 0; j < nodes.length; j++) {
            var objNode=labelsMap[nodes[j].nom];
            if(!objNode){
                var objNode={
                    id:ids[j],
                    label:nodes[j].nom,
            ...
            if(j<nodes.length-1){
                var startLabel= nodes[j].nom;
                var endLabel= nodes[j+1].nom;
    ...
    loadLabelsCoordinates(subGraph,labelsMap)
...
function draw(dataNodes,dataRels) {
...
function loadLabelsCoordinates(subGraph,labelsMap){
    ...
    submitMongo(query, data, function(d){
